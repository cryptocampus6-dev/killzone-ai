...
def load_data():
    default_data = {
        ...
        "sent_morning": False,
        "sent_goodbye": False # Goodbye මැසේජ් එක එකපාරයි යන්නේ
    }
...
def analyze_ultimate(df, btc_trend):
    ...
    high_max = df['high'].max(); low_min = df['low'].min()
    score = 50; methods_hit = []
    is_downtrend = curr['close'] < curr['ema200']; is_uptrend = curr['close'] > curr['ema200']
    if curr['rsi'] < 30: 
    ...
    if abs(curr['close'] - fib_618) / curr['close'] < 0.005: score += 15; methods_hit.append("Fibonacci Golden")
    local_high = df['high'].iloc[-10:-1].max(); local_low = df['low'].iloc[-10:-1].min()
    ...
    if curr['low'] < df['low'].iloc[-10:-1].min() and curr['close'] > curr['open']: score += 15; methods_hit.append("ICT Liq Grab (Buy)")
    elif curr['high'] > df['high'].iloc[-10:-1].max() and curr['close'] < curr['open']: score -= 15; methods_hit.append("ICT Liq Grab (Sell)")
    if curr['close'] > prev['close'] and df['volume'].iloc[-1] > df['volume'].mean(): score += 10; methods_hit.append("Vol Pump")
    elif curr['close'] < prev['close'] and df['volume'].iloc[-1] > df['volume'].mean(): score -= 10; methods_hit.append("Vol Dump")
    ...
    if 18 <= now_lk.hour <= 20: methods_hit.append("News Zone ⚠️")
    if curr['atr'] > df['atr'].mean(): score += 5 
    swing_low = df['low'].iloc[-15:].min(); swing_high = df['high'].iloc[-15:].max() 
...
def run_scan():
    ...
    for i, coin in enumerate(coins_list):
        ...
        try:
            ...
            if not df.empty:
                ...
                time.sleep(0.1)
                if sig != "NEUTRAL" and st.session_state.daily_count < MAX_DAILY_SIGNALS:
                    ...
                    if sig == "LONG":
                        sl = raw_sl; dist = price - sl; tp_dist = dist * 2.0
                        tps = [price + (tp_dist * x * 0.6) for x in range(1, 5)] 
                        emoji_circle = "🟢"; direction_txt = "Long"
                    else:
                        sl = raw_sl; dist = sl - price; tp_dist = dist * 2.0
                        tps = [price - (tp_dist * x * 0.6) for x in range(1, 5)]
    ...
    status_area.empty(); st.success("Scan Complete!")
...
with tab1:
    if st.session_state.bot_active:
        ...
        else: st.warning("💤 SLEEPING"); time.sleep(10); st.rerun()
