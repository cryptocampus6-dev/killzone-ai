...
# --- 5. MAIN ENGINE ---
def main():
    st.title("👻 GHOST PROTOCOL : 24/7 VIP ENGINE")
    ...
    # Auto-Start Logic: පැය 24ම වැඩ කරයි
    ...
    with tab1:
        ...
        if st.session_state.engine_running:
            ...
            while st.session_state.engine_running:
                ...
                # 🌙 Night Mode එක ඉවත් කර ඇත - පැය 24ම වැඩ කරයි
                status_placeholder.success("✅ ENGINE RUNNING (24/7 Unstoppable)")

                time_diff = (now_live - last_scan_time).total_seconds()
                
                if time_diff >= 900: # විනාඩි 15කට සැරයක් ස්කෑන් කරයි
                        ...
                        try:
                            ...
                            if not df.empty:
                                ...
                                if (sig == "LONG" and score >= 75) or (sig == "SHORT" and score <= 25):
                                    send_telegram("💎 VIP ALERT", sticker=True)
                                    sl_dist = atr * 1.5
                                    if sig == "LONG":
                                        sl = price - sl_dist
                    ...
                    log_box.info(f"⏳ Next Scan in {max(0, mins_left)} minutes...")
                    time.sleep(1)
